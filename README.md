...
**Palavras**: Lista local curada (266 substantivos e adjetivos pt-BR em `palavras_base.txt`). Filtros automáticos removem verbos, pronomes, nomes próprios e palavras inadequadas. Na primeira execução, baixa ~500 palavras da web para o cache. A cada 10 dias, 25% do cache é rotacionado com novas palavras da web em background.

**Dicas via LLM**: Sistema de mirrors com 3 camadas:
1. **OpenRouter** (gratuito, com API key) — modelos `openai/gpt-oss-120b:free` e `nvidia/nemotron-3-nano-30b-a3b:free`
...
```
forca.py            # Código principal (~1020 linhas)
